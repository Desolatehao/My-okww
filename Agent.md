...
`参考资料/` 放的是各条轴开发时读的原始资料，不是交付物，删掉就没有对照依据了。
它按轴分文件夹，当前这条轴的在 `参考资料/弗卜维涉及资料/`，详细说明见该文件夹的 `README.md`。
以后每写一条新轴，就在 `参考资料/` 下并排新建一个 `<轴名>涉及资料/`。

- `参考资料/弗卜维涉及资料/Augusta_Baizhi_Buling_4_1.0.0/` 及同名 zip —— 卜灵参数来源（实现层等待值，非逐帧数据）。
- `参考资料/弗卜维涉及资料/Cantarella_Mornye_Phrolova_楽山_1.0.0/` —— 弗洛洛参数来源（作者 楽山，含完整帧表）。
- `参考资料/弗卜维涉及资料/Hiyuki_Lucilla_Verina_a38999_1.0.0.zip` —— 维里奈参数来源（`ATTACK_INTERVAL = 0.1`）。
- `参考资料/弗卜维涉及资料/弗坎洛轮椅-*.wwcombo.json` —— wwcombo-chart 连招图，动作编排参照。

`TIMING.md` 的「证据来源」一节直接引用上面这些路径；1.0.6 – 1.1.1 的历史快照里写的还是移动前的
根目录路径，那些是冻结记录，不再回改。
